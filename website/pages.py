...
PLAYER_STATS_QUERIES = {
    ...
    "HCR": "SELECT (SUM(HucksCompleted) + SUM(HucksIncompleteForced) + SUM(HucksIncompleteUnforced) + SUM(HucksIncompleteOther)) / NULLIF(SUM(HucksCompleted), 0) FROM cutstats WHERE players LIKE '%player%';",
}
