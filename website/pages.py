...
PLAYER_STATS_QUERIES = {
    ...
    # "Hold %" : "SELECT (SELECT COUNT(*) FROM cutstats WHERE players LIKE '%player%' AND scored LIKE 'TRUE' AND pulled LIKE 'FALSE') / NULLIF((SELECT COUNT(*) FROM cutstats WHERE players LIKE '%player%' AND pulled LIKE 'FALSE'), 0);",
    # "Hold %" : "SELECT (COUNT(*) FILTER (WHERE scored LIKE 'TRUE')) / NULLIF(COUNT(*), 0) FROM cutstats WHERE players LIKE '%player%' AND pulled LIKE 'FALSE';",
    "Hold %" : "SELECT NULLIF(COUNT(*), 0) FROM cutstats WHERE players LIKE '%player%' AND pulled LIKE 'FALSE';",
