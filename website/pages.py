...
PLAYER_STATS_QUERIES = {
    ...
    "HCR": "SELECT ROUND(((SUM(HucksCompleted) + SUM(HucksIncompleteForced) + SUM(HucksIncompleteUnforced) + SUM(HucksIncompleteOther)) / NULLIF(SUM(HucksCompleted), 0))::numeric, 2) FROM cutstats WHERE players LIKE '%player%';",
}
